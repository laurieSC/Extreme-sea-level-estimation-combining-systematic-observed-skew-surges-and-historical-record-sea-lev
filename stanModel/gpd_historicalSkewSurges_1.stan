...
data {
  ...
  real<lower=u> u_H;      // Threshold of historical skew surge
  real w_H;                 // Historical duration
  real<lower=u_H> xH;      // Historical skew surges
}
...
model {
  ...
  // Log-probability of observing 1 skew surge exceeding u during the historical duration w_H (years)
  Ph = log(lambda*w_H) - lambda*w_H*(1 - gpd_cdf(u_H, u, sigma, xi));
  
  // Density of the historical skew surge
  f = gpd_df(xH, u, sigma, xi);
  
  // Log-likelihood of the historical skew surge
  Lh = Ph + log(f);
