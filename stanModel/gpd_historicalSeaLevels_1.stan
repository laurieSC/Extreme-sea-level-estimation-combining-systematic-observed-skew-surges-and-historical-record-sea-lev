...
data {
  ...
  vector[nn] xH_low;             // Historical sea level (larger than eta_H) - tide
  vector[nn] xH_up;              // [Historical sea level (larger than eta_H)]*1.01 - tide
  real P_xH_low;                 // Probability of non exceedance of xH_low
  real P_xH_up;                  // Probability of non exceedance of xH_up
}
...
model {
  ...
  // Log-probability of observing (N - 1) sea levels not exceeding eta_H during the historical duration w_H (years)
  Ph = (N - 1)*log(p*P_eta_H + (1 - p)*gpd_cdf(eta_H_less_tide_d, tide_d, u, sigma, xi));
  
  // Density of the historical sea levels exceeding eta_H
  g = ((p*P_xH_up + (1 - p)*gpd_cdf(xH_up, tide_d, u, sigma, xi)) - (p*P_xH_low + (1 - p)*gpd_cdf(xH_low, tide_d, u, sigma, xi)))/(xH_up[1] - xH_low[1]);
